...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		FileOutputStream outputStream = new FileOutputStream(new File(
				"compressed.txt"));
		FileOutputStream codeStream = new FileOutputStream(
				new File("codes.txt"));
		...
		try {
			...
			for (int i = 0; i < str.length(); i++) {
				...
				if (codeBuffer.length() > 256) {
					while (codeBuffer.length() > 8) {
						int chr = Integer.parseInt(codeBuffer.substring(0, 8),
								2);
			...
			System.out.println("Uncompressed file size: " + asciiCost / 8
					+ " bytes");
			System.out.println("Compressed file size: " + compressedCost / 8
					+ " bytes");
			System.out.println("Compression ratio: " + compressedCost * 100
					/ asciiCost + "%");
